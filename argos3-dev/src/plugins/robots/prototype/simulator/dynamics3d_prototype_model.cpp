...
namespace argos {
   ...
   std::shared_ptr<btCollisionShape> CDynamics3DPrototypeModel::RequestShape(const CPrototypeLinkEntity& c_link_entity) {
      ...
      std::cerr << "HE = " << to_string(cHalfExtents) << std::endl;
      ...
      case CPrototypeLinkEntity::EGeometry::CONVEX_HULL:
         ...
         for(const CVector3& c_point : c_link_entity.GetConvexHullPoints()) {
            vecConvexHullPoints.emplace_back(c_point.GetX(),
                                             c_point.GetZ(),
                                            -c_point.GetY());
            std::cerr << to_string(vecConvexHullPoints.back()) << std::endl;
         }
   ...
   CDynamics3DMultiBodyObjectModel::CAbstractBody::SData
      ...
      std::cerr << c_link_entity.GetId() << std::endl;
      ...
      std::cerr << "Prototype: cStartTransform = " << to_string(cStartTransform) << std::endl;
      std::cerr << "COM = " << to_string(cCenterOfMassOffset) << std::endl;
      std::cerr << "mass = " << fMass << std::endl
                << "friction = " << fFriction << std::endl;
   ...
   CDynamics3DPrototypeModel::CDynamics3DPrototypeModel(CDynamics3DEngine& c_engine,
      ...
      for(auto& kv : c_entity.GetEmbodiedEntity().GetAnchors()) {
         std::cerr << kv.first << kv.second->OffsetPosition << "/" << kv.second->OffsetOrientation << std::endl;
      }
      ...
      while(! vecJointsToAdd.empty()) {
         ...
         for(CPrototypeJointEntity::TVectorIterator it_joint = std::begin(vecJointsToAdd);
            ...
            */
            ...
            std::cerr << "cParentOffsetTransform.getOrigin() = " << to_string(cParentOffsetTransform.getOrigin()) << std::endl;
            std::cerr << "-cChildOffsetTransform.getOrigin() = " << to_string(-cChildOffsetTransform.getOrigin()) << std::endl;
            std::cerr << "cParentToChildRotation = " << to_string(cParentToChildRotation) << std::endl;
...
/*
                                        std::cerr << "s_joint.Child.GetIndex() = " << s_joint.Child.GetIndex() << std::endl;
                                        std::cerr << "s_joint.Child.GetData().Mass = " << s_joint.Child.GetData().Mass << std::endl;
                                        std::cerr << "s_joint.Child.GetData().Inertia = " << to_string(s_joint.Child.GetData().Inertia) << std::endl;
                                        std::cerr << "s_joint.Parent.GetIndex() = " << s_joint.Parent.GetIndex() << std::endl;
                                        std::cerr << "s_joint.ParentToChildRotation = " << to_string(s_joint.ParentToChildRotation) << std::endl;
                                        std::cerr << "s_joint.ParentOffset = " << to_string(s_joint.ParentOffset) << std::endl;
                                        std::cerr << "s_joint.ChildOffset = " << to_string(s_joint.ChildOffset) << std::endl;
                                        std::cerr << "s_joint.DisableCollision = " << (s_joint.DisableCollision ? "true" : "false") << std::endl;
*/
         ...
         case CPrototypeJointEntity::EType::REVOLUTE:
            m_cMultiBody.setupRevolute(s_joint.Child->GetIndex(),
                                       ...
                                       std::cerr << to_string(s_joint.Axis) << std::endl;
            break;
